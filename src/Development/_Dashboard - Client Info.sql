...
-- DECLARE @OnInternet   AS NVARCHAR(3)  = 'SMS0001' --You could make this into a multiselection in the report

SELECT
    Device            = (
        IIF(
            SystemNames.Resource_Names0 IS NOT NULL, UPPER(SystemNames.Resource_Names0)
            , IIF(Systems.Full_Domain_Name0 IS NOT NULL, ClientBaseline.Name + N'.' + Systems.Full_Domain_Name0, ClientBaseline.Name)
        )
    )
    ...
    , AccessMP        = CNAccessMP
    , IsOnInternet    = IIF(ClientBaseline.CNIsOnInternet = 1, N'Yes', N'No')
FROM
