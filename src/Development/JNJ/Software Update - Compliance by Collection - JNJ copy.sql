...
AS (
    ...
    --     AND UpdateCIs.IsSuperseded IN (0,1)                                -- Filter on Superseded
    --     AND UpdateCIs.IsEnabled IN (0,1)                                   -- Filter on Deployment Enabled
...
SELECT
     HealthStates = (
        CASE
			WHEN #Result.OperatingSystem LIKE '%Windows 7%' THEN 'Servicing Expired'
			WHEN #Result.OperatingSystem LIKE '%Windows 10 Pro%' THEN 'Non-Enterprise Version'
            WHEN #Result.HealthStates IS NULL THEN N'Unknown'
            WHEN #Result.HealthStates = 0 THEN N'Healthy'
            ELSE
                STUFF((
                    SELECT ', ' + hs.StateName
                    FROM @HealthState hs
                    WHERE (hs.BitMask & #Result.HealthStates) <> 0
                    ORDER BY hs.BitMask
                    FOR XML PATH(''), TYPE
	...
	, Compliance = (
        CASE
            WHEN #Result.Missing = 0 AND #Result.ClientState = 'Active/Pass' AND #Result.OSServicingState = 'Current' THEN N'Compliant'
            WHEN (#Result.Missing > 0 AND #Result.Missing IS NOT NULL) OR (#Result.Missing IS NOT NULL AND #Result.OSServicingState <> 'Current') THEN N'Non-Compliant'
            ELSE N'Unknown'
		END
     )
    , Missing
