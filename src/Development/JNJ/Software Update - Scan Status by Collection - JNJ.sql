...
SELECT SubString FROM fn_SplitString(@HealthThresholds, N',');
...
SELECT
    ...
    , CatalogVersionsBehind   = IIF(SyncSourceInfo.SyncCatalogVersion = N'', NULL, SyncSourceInfo.SyncCatalogVersion - UpdateScan.LastScanPackageVersion)
...
FROM fn_rbac_FullCollectionMembership(@UserSIDs) AS CollectionMembers
    ...
    LEFT JOIN fn_rbac_UpdateScanStatus(@UserSIDs) AS UpdateScan ON UpdateScan.ResourceID = CollectionMembers.ResourceID
        AND (
            @UpdateSearchID = UpdateScan.UpdateSource_ID OR @UpdateSearchID = N''
        )
