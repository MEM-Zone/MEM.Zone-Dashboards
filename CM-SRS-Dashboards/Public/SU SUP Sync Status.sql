/*
.SYNOPSIS
    Gets SCCM SUP Sync Status.
.DESCRIPTION
    Gets SCCM SUP Sync Status with state message details.
.NOTES
...
FROM vSMS_SUPSyncStatus AS SUPSyncStatus
    CROSS APPLY (
        ...
        WHERE StatusMessage.MessageID = SUPSyncStatus.LastSyncState
            AND StatusMessage.Time >= DATEADD(MINUTE, -2, SUPSyncStatus.LastSyncStateTime) -- Workaround for the time difference between StatusMessage.Time and SUPSyncStatus.LastSyncStateTime (Should be equal in a perfect world)
